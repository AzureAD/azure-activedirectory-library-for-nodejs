...
declare module "adal-node" {
   ...
   * @memberOf Logging
   * @param {LOGGING_LEVEL} level The level of this log entry.
   ...
   */
  type LoggingCallback = (level: LOGGING_LEVEL, message: string, error?: Error) => void;

  /**
   * @typedef LoggingOptions
   * @memberOf Logging
   * @property {LoggingCallback} [log] The function to call when ADAL generates a log entry.
   * @property {LOGGING_LEVEL} [level] The maximum level of log entries to generate.
   */
  ...
  /**
   * @interface MemoryCache - Describes the in memory implementation of the token cache.
   */
