 ...
 * @param {Array} [entries] If the find operation was successful then the matched entries should be returned here.
 */
 ...
 * @param {bool}   [validateAuthority]   Turns authority validation on or off.  This parameter defaults to true.
 ...
 * @param  {string}   refreshToken                        A refresh token returned in a token response from a previous invocation of acquireToken.
 * @param  {string}   clientId                            The OAuth client id of the calling application.
 ...
 *                                                        to work unchanged.)
 * @param  {string}   resource                            The OAuth resource for which a token is being requested.  This parameter is optional and can be set to null.
 * @param  {AcquireTokenCallback}   callback              The callback function.
 */
...
/**
 * Gets a new access token using a device code.
...
AuthenticationContext.prototype.cancelRequestToGetTokenWithDeviceCode = function (userCodeInfo, callback) {
    ...
    if (!this._tokenRequestWithUserCode || !this._tokenRequestWithUserCode[userCodeInfo[constants.UserCodeResponseFields.DEVICE_CODE]]) {
       callback(new Error('No acquireTokenWithDeviceCodeRequest existed to be canceled')); 
       return;
    }
