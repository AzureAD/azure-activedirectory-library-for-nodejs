...
/**
 * Gets a token for given scope.
 * @param {array}    scope                               The OAuth scopes for which a token is being request.
 ...
 */
AuthenticationContext.prototype.acquireToken = function(scope, userId, clientId, policy, callback) {
  argument.validateCallbackType(callback);
  try {
    argument.validateArrayParameter(scope, 'scope');
  ...
  this._acquireToken(callback, function() {
    var tokenRequest = new TokenRequest(this._callContext, this, clientId, scope, policy);
 ...
 * @param {AcquireTokenCallback}   callback              The callback function.
 */
AuthenticationContext.prototype.acquireTokenWithAuthorizationCode = function(authorizationCode, redirectUri, scope, clientId, clientSecret, policy, callback) {
  argument.validateCallbackType(callback);
  try {
    argument.validateArrayParameter(scope, 'scope');
  ...
  this._acquireToken(callback, function() {
    var tokenRequest = new TokenRequest(this._callContext, this, clientId, scope, policy, redirectUri);
 ...
 *                                                        to work unchanged.)
 * @param  {array}   scope                                The OAuth scope for which a token is being request. 
 * @param  {AcquireTokenCallback}   callback              The callback function.
 */
AuthenticationContext.prototype.acquireTokenWithRefreshToken = function(refreshToken, clientId, clientSecret, scope, policy, redirectUri, callback) {
  ...
  var clientSecretPresent = (7 === arguments.length);
  var actualClientSecret = clientSecretPresent ? clientSecret : null;
  var actualScope = clientSecretPresent ? arguments[3] : arguments[2];
  var actualPolicy = clientSecretPresent ? arguments[4] : arguments[3];
  var actualRedirectUri = clientSecretPresent ? arguments[5] : arguments[4];
  var actualCallback = clientSecretPresent ? arguments[6] : arguments[5];
  ...
  this._acquireToken(callback, function() {
    var tokenRequest = new TokenRequest(this._callContext, this, clientId, actualScope, actualPolicy, actualRedirectUri);
