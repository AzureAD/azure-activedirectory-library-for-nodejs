...
function createRequestHandler(operationMessage, log, errorCallback, successCallback) {
  return function (err, response, body) {
    log.verbose("Response in utils createRequestHandler: " + response);
...
/**
* Error generating API when the http response is not 200
* to centralize error handling in one place.
* @static
* @memberOf Util
* @param {string} operationMessage  A message to be prepended to logged error strings.  This should be something like 'Mex Request'
*                                   and summarize the purpose of the http request.
* @param {object} log               A Logger object being used by the calling component.
* @param {error}                    error parameter generated for the callback to be Called in the event of an error.
*/
function createHttpError(operationMessage, log, response) {
  log.verbose("Response in utils createHttpError: " + response);
  logReturnCorrelationId(log, operationMessage, response);
  
  util.logReturnCorrelationId(log, 'User Realm Discovery', response);
  var returnErrorString = 'User Realm Discovery' + ' request returned http error: ' + response.status;
  var errorResponse;
  if (response.data) {
    returnErrorString += ' and server response: ' + response.data;
    try {
  ...
  return {
    errorLog: log.createError(returnErrorString, true),
