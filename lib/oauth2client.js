...
TOKEN_RESPONSE_MAP[OAuth2ResponseParameters.SCOPE] = TokenResponseFields.SCOPE;
 ...
 * @return {URL}
 */
OAuth2Client.prototype._createTokenUrl = function (policy) {
  var tokenUrl = url.parse(this._tokenEndpoint);

  var parameters = {};
  parameters.slice = 'testslice';
  parameters.msaproxy = true;
  parameters[OAuth2Parameters.AAD_API_VERSION] = '1.0';

  if (policy){
    parameters[OAuth2Parameters.POLICY] = policy;
  }
...
OAuth2Client.prototype.getToken = function (oauthParameters, callback) {
  ...
  var policy = null;
  if (oauthParameters.hasOwnProperty(OAuth2Parameters.POLICY)){
    policy = oauthParameters[OAuth2Parameters.POLICY];
    delete oauthParameters[OAuth2Parameters.POLICY];
  }
  var tokenUrl = self._createTokenUrl(policy);
