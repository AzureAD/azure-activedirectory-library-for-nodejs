...
*/
UserRealm.prototype._parseDiscoveryResponse = function(response, callback) {
  this._log.verbose('Discovery response:\n' + response, true);

  var discoveryResponse;
  try {
    discoveryResponse = response.data;
  } catch (err) {
    this._log.verbose('In parse error:\n' + response.data, true);
    callback(this._log.createError('Parsing realm discovery respone JSON failed: ' + response, true));
    return;
  }

  var accountType = this._validateAccountType(discoveryResponse['account_type']);
  if (!accountType) {
  ...
  if (this._accountType === AccountType.Federated) {
    var protocol = this._validateFederationProtocol(discoveryResponse['federation_protocol']);
    ...
    this._federationMetadataUrl = discoveryResponse['federation_metadata_url'];
    this._federationActiveAuthUrl = discoveryResponse['federation_active_auth_url'];
  }
...
UserRealm.prototype.discover = function (callback) {
  ...
  axios.get(userRealmUrl, options).then((response) => {
    ...
    if (!util.isHttpSuccess(response.status)) {
      const errorHandler = util.createHttpError('User Realm Discovery', this._log, response);
      callback(errorHandler.errorLog, errorHandler.errorResponse);
    }
    
    // status = 200: success case
    self._parseDiscoveryResponse(response, callback);
  }).catch((error) => {
    // status >= 400: error case
    this._log.error('User Realm Discovery' + ' request failed with', error, true);
    callback(error);
  });
};
