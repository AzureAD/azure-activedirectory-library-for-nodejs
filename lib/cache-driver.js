...
// the parameters (authority, scope, clientId, userId), in any combination.  They must be able find, add, and remove
...
//
// there be a single cache entry per (authority, scope, clientId)
 ...
 * @param {string}  [scope]       The scope for which to obtain the token if it is different from the original token.
 ...
 */
function CacheDriver(callContext, authority, scope, clientId, policy, cache, refreshFunction) {
...
CacheDriver.prototype._getPotentialEntries = function(query, callback) {
  ...
  if (query.policy) {
    potentialEntriesQuery['policy'] = query.policy;
  }
...
CacheDriver.prototype._loadSingleEntryFromCache = function(query, callback) {
  ...
  this._getPotentialEntries(query, function(err, potentialEntries) {
    ...
    // For given client id, user id, and authority, there should be only one returned. With Convergence, scope will be stored instead of resource. 
    if (potentialEntries && 0 < potentialEntries.length) {
      ...
      for (var index = 0; index < potentialEntries.length; ++index) { 
         if (_entryContainsGivenScopes(potentialEntries[index], self._scopes)) {
            resourceSpecificEntries.push(potentialEntries[index]);
         }          
      }
...
CacheDriver.prototype._acquireNewTokenFromMrrt = function(entry, callback) {
  var self = this;
  this._refreshFunction(entry, this._scopes, function(err, tokenResponse) {
    if (err) {
      callback(err);
      return;
    }
    ...
    var newEntry = self._createEntryFromRefresh(entry, tokenResponse);
    self.manageCache(newEntry, function(err) {
      if (err) {
...
CacheDriver.prototype._augmentEntryWithCacheMetadata = function(entry) {
  ...
  if (isMRRT(entry)) {
    ...
    entry.scope = _parseScope(this._scopes);
  }

  if (this._policy && !_.isEmpty(this._policy)){
    entry.policy = this._policy;
  }
...
/**
 * Manages the cache. If there is already an entry with the same client id, user id and authority, replace
 * the existing entry, otherwise add an new entry in the cache. 
 */ 
CacheDriver.prototype.manageCache = function (entry, callback) { 
   var self = this;
   var query = {};
    if (this._clientId) {
        query[METADATA_CLIENTID] = this._clientId;
    }

    if (this._authority){
        query[METADATA_AUTHORITY] = this._authority;
    }

    if (entry[TokenResponseFields.USER_ID]) {
        query[TokenResponseFields.USER_ID] = entry[TokenResponseFields.USER_ID];
    }

    if (this._policy){
        query['policy'] = this._policy;
    }
    ...
    this._getPotentialEntries(query, function (err, potentialEntries) {
        ...
        if (0 === potentialEntries.length) {
            self.add(entry, callback);
        }
        else { 
           // With the current cache logic, there should only be one entry
           // per client id, user id, and authority
           self._augmentEntryWithCacheMetadata(entry);
           var potentialEntry = potentialEntries[0]; 
           var newEntry = _.clone(potentialEntry);
           newEntry = _.extend(newEntry, entry); 

           self._replaceEntry(potentialEntry, newEntry, callback);
        }
     });
};

/**
 * Checks to see if given entry contain the given list of scopes. 
 */
function _entryContainsGivenScopes(potentialEntry, scope) {
    if (scope) {
        var scopeForEntry = potentialEntry['scope'];
        for (var i = 0; i < scope.length; ++i) {
            if (scopeForEntry.indexOf(scope[i]) === -1) {
                return false;
            }
        }
    }
