...
CacheDriver.prototype._acquireNewTokenFromMrrt = function(entry, callback) {
  var self = this;
  this._refreshFunction(entry, this._scope, function(err, tokenResponse) {
    if (err) {
      callback(err);
      return;
    }
...
CacheDriver.prototype._augmentEntryWithCacheMetadata = function(entry) {
  ...
  if (isMRRT(entry)) {
    ...
    entry.scope = _parseScope(this._scope);
  }
...
/**
 * Checks to see if given entries contain the given list of scopes. 
 */
