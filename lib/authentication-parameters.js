...
exports.createAuthenticationParametersFromUrl = function(url, callback, correlationId) {
  ...
  try {
    ...
    axios.get(challengeUrl, options).then((response) => {
      logger.verbose("Response in authentication-parameters: " + response);
      
      // status >= 300 && < 400
      if (!util.isHttpSuccess(response.status)) {
        const errorHandler = util.createHttpError('Authentication Parameters', this._log, response);
      ...
      var parameters;
      try {
        parameters = exports.createAuthenticationParametersFromResponse(response.data);
      } catch(creationErr) {
      ...
      logger.error('Authentication parameters http get failed.', error, true);
      callback(error);
      return;
    });
  } catch(err) {
