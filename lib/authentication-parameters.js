 ...
 * @constructor
 * @param {string} authorizationUri The URI of an authority that can issue tokens for the
...
/**
 * The URI of an authority that can issue tokens for the resource that issued the challenge.
 * @instance
 ...
 * @param {AuthenticationParameters} [parameters]   On a successful request returns a {@link AuthenticationParameters}.
 */
  ...
  try {
    ...
    request.get(challengeUrl, options, function(err, response) {
      ...
      try {
        ...
        logger.error('Unable to parse response into authentication paramaters.', creationErr);
