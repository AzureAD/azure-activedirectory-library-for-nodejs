 ...
 * @param {string} scope            The scope for a which a token should be requested from the authority.
 */
function AuthenticationParameters(authorizationUri, scope) {
  this._authorizationUri = authorizationUri;
  this._scope = scope;
}

/**
 * The URI of an authority that can issues tokens for the scope that issued the challenge.
 * @instance
...
/**
 * The scope for a which a token should be requested from the authority.
 * This property may be undefined if the scope was not returned in the challenge.
 * @instance
 * @memberOf AuthenticationParameters
 * @type {string}
 * @name authorizationUri
 */
Object.defineProperty(AuthenticationParameters.prototype, 'scope', {
  get : function() {
    return this._scope;
  }
});
...
// WWW-Authenticate : Bearer authorization_uri="https://login.windows.net/mytenant.com/oauth2/authorize",Resource_id="00000002-0000-0000-c000-000000000000/Mail.Read"
...
exports.createAuthenticationParametersFromHeader = function(challenge) {
  ...
  var scope = challengeParameters[consts.SCOPE];

  return new AuthenticationParameters(authorizationUri, scope);
};
...
exports.createAuthenticationParametersFromResponse = function(response) {
  ...
  if (!challenge) {
    throw new Error('The response does not contain a WWW-Authenticate header that can be used to determine the authority_uri and scope.');
  }
