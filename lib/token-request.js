...
TokenRequest.prototype.getTokenFromCacheWithRefresh = function (userId, callback) {
  ...
  self._userId = userId;
  self._cacheDriver = self._createCacheDriver();
  var cacheQuery = self._createCacheQuery();
  self._cacheDriver.find(cacheQuery, function (err, token) {
    if (!token) {
      err = err || 'Entry not found in cache.';
      self._log.warn('Attempt to look for token in cache resulted in Error: ' + err.stack);
      return callback(self._log.createError(err));
    }
    self._log.info('Returning cached token.');
