 ...
 * @param {string} policy
 */
function TokenRequest(callContext, authenticationContext, clientId, scope, policy, redirectUri) {
  ...
  this._policy = (policy && !_.isEmpty(policy)) ? policy : null;
 ...
 */
TokenRequest.prototype._getTokenWithTokenResponse = function(entry, scope, callback) {
  this._log.verbose('Called to refresh a token from the cache.');
  var refreshToken = entry[TokenResponseFields.REFRESH_TOKEN];
  this._getTokenWithRefreshToken(refreshToken, scope, null, callback);
};
...
TokenRequest.prototype._createCacheQuery = function() {
  ...
  if (this._scope) {
    ...
    this._log.verbose('No scopes passed for cache query.');
  }

  if (this._policy){
    query.policy = this._policy;
  } else {
    this._log.verbose('No policy passed for cache query.');
  }
  ...
  this._cacheDriver.find(cacheQuery, function(err, token) {
    ...
    if (!token) {
      ...
      getTokenFunc.call(self, function(err, tokenResponse) {
        ...
        self._cacheDriver.manageCache(tokenResponse, function() {
...
/**
 * Parses the given Array of scopes to be a string delimite by space. 
 * @private
...
TokenRequest.prototype._createOAuthParameters = function(grantType) {
  ...
  _addParameterIfAvailable(oauthParameters, OAuth2Parameters.SCOPE, _parseScope(this._scope));

  _addParameterIfAvailable(oauthParameters, OAuth2Parameters.POLICY, this._policy);
 ...
 * @param  {AcquireTokenCallback} callback
 */
TokenRequest.prototype._getTokenWithRefreshToken = function(refreshToken, scope, clientSecret, callback) {
