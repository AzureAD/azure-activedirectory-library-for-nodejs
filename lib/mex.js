  ...
  request.get(this._url, options, util.createRequestHandler('Mex Get', this._log, callback,
    function(response, body) {
      try {
        ...
        self._log.error('Failed to parse mex response into DOM', err);
        callback(err);
      }
    })
  );
...
* @returns {object} a map of binding id's to policy id's.
*/
...
Mex.prototype._selectSingleMatchingPolicy = function(policies) {
  ...
  if (!matchingPolicies) {
    this._log.warn('no policies found with the url');
    return;
  }
