...
Mex.prototype.discover = function (callback) {
  ...
  axios.get(this._url, options).then((response) => {
    ...
    if (!util.isHttpSuccess(response.status)) {
      ...
      var returnErrorString = 'Mex Get' + ' request returned http error: ' + response.statusCode;
      var errorResponse;
      if (response.data) {
    ...
    if (error.headers && error.headers['client-request-id']) {
      this._log.info('Mex Get' + 'Server returned this correlationId: ' + error.headers['client-request-id'], true);
    }
    this._log.error('Mex Get' + ' request failed with', error, true);
    callback(error);
  });
};
