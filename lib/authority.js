...
Authority.prototype._performDynamicInstanceDiscovery = function(callback) {
  try {
    ...
    axios.get(discoveryEndpoint, getOptions).then((response) => {
      // success case (2xx)
      var discoveryResponse = JSON.parse(response.data);
      ...
      if (!util.isHttpSuccess(response.status)) {
        ...
        var returnErrorString = 'Instance Discovery' + ' request returned http error: ' + response.status;
        var errorResponse;
        if (response.data) {
        ...
        callback(this._log.createError(returnErrorString, true), errorResponse);
      }
    }).catch((error) => {
      // errors ( >= 400x)
      if (error.headers && error.headers['client-request-id']) {
        this._log.info('Instance Discovery' + 'Server returned this correlationId: ' + error.headers['client-request-id'], true);
      }
      this._log.error('Instance Discovery' + ' request failed with', error.status, true);
      callback(error);
    });
  } catch(e) {
    callback(e);
