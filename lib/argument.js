...
var argumentValidation = {
  ...
  validateArrayParameter : function (param, name) {
    if (!param) { 
       throw new Error('The ' + name + ' parameter is required.');
    }

    if (!_.isArray(param)) { 
      throw new Error('The ' + name + ' parameter must be of type Array.');
    }

    if (!param.every(function (element, index, array) { return _.isString(element);})) { 
       throw new Error('The ' + name + ' parameter must be consisted of an array of String');
    }
  }
};
